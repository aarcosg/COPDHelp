        ...
        for (final Class<? extends RealmModel> c : configuration.getRealmObjectClasses()) {
            ...
            if (objectSchema != null && objectSchema.hasPrimaryKey()) {
                ...
                try {
                    ...
                    Log.d(getClass().getSimpleName(), String.format("error while getting number primary key %s " +
                            " for %s",PRIMARY_KEY_FIELD, c.getName()), ex);
                }
                if (keyValue == null) {
                    Log.w(getClass().getSimpleName(), String.format("can't find number primary key %s " +
